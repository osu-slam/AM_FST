...
% Traditional ISSS
p.TR     = 2.000; 
p.epiNum = 5;  % testing hybrid_isss version!

% Timing
p.runsMax = 4; % New design has more stimuli and longer runs
p.events  = 8; % Events per block, needs updating. 
p.sentences  = 8; % how many sentence stimuli per block?
p.structures = 8; % how many sentence structures?
...
%                 % (clear, easy, hard)
% p.rate = [0.75 1.25]; % make sure to put in ascending order
...
dir_stim_all = fullfile(dir_stim, 'UTD_demo'); 
% dir_stim_all = fullfile(dir_stim, 'YA_FST_v4_all_babblematch_rms'); 
...
%% Create keys
% Just testing with 8 stimuli per run. Commenting this out because it's all
% over-engineered... right?

%%% ORIGINAL NOTES
% Each block consists of 24 sentences. There will be 12 OR and 12 SR
% sentences (pre-selected) based on 12 sentence structures. There will be 8
% clear, 8 easy babble (high SNR), and 8 hard babble (low SNR) sentences.
% There will be 12 slow and 12 fast sentences. 
%%%
...
% 
% ntrial = p.runsMax * 2 * (length(cla)) * length(p.rate) * 2;
% 
...
%     for ss=1:2 % syntax
%         for cc=1:length(cla)
%             for aa=1:length(p.rate)
%                 for nn = 1:2 % repetitions
%                     in = in+1;
%                     cond(in,1:4) = [runs(rr), syn(ss), cla(cc), spr(aa)];
%                 end
%             end
%         end
%     end
...
% 
% for ss=1:2 % syntax
%     str = CreateLatinSquare(p.runsMax*2); % number of runs, 2 repetitions
%     str = str(randperm(p.runsMax*2),:);
%     in = 0;
%     for cc=1:length(cla) % clarity
%         for aa=1:length(p.rate) % speech rate
%             in=in+1;
%             idx = ( cond(:,2)==syn(ss) & cond(:,3)==cla(cc) & cond(:,4)==spr(aa) );
%             cond(idx,5) = str(in,:);
%         end
%     end
% end
...
% gen = "*";
% for tt=1:ntrial
%     if str2double(cond(tt, 5)) < 10
%         zz = "00";
%     else
%         zz = "0";
%     end
%     cond(tt,6) = strcat(zz,cond(tt,5),"_f",cond(tt,2),gen,"_",cond(tt,4));
% end
% 
...
%     
%     noise1 = [num2str(rr), "N", "N", "N", "N", "noise_rate_075"];
%     noise2 = [num2str(rr), "N", "N", "N", "N", "noise_rate_125"];
%     silence = [num2str(rr), "S", "S", "S", "S", "silence01"];
%     
%     thisblock = [thisblock; noise1; noise2; silence; silence]; 
...
%%% key_events is bypassed for testing
...
%%% Code for testing
files = dir(fullfile(dir_stim_all, '*.wav')); files = {files.name};
files = fullfile(dir_stim_all, files)';

for ii = 1:p.events
    [tempAudio, fs_events(ii, 1)] = audioread(files{ii}); 
    ad_events{ii, 1} = [tempAudio'; tempAudio']; 
end

ad_events = repmat(ad_events(:, 1), [1, p.runsMax]);
fs_events = fs_events(:, 1);
%%%
...
pahandle = PsychPortAudio('Open', 3, [], [], fs); 
% 5 in lab, 18 at CCBBI, 3 at home
...
% large, to get a latinsquare with interleve11112222 style: CreatLatinSquare(n,1).
% by Niki---2013/3/18
