function [wave, fs] = jp_vocode_mh(soundvec, num_channels, fs, opts)
% JP_VOCODE Vocode a sound with the option to shift frequencies.
%   10/10/19 -- Edited to vocode a vector, not sound file. MH
% 
...
% assert(min(size(soundvec))==1, 'Sound file must be mono.');
assert(num_channels > 0, 'Must have at least 1 channel.');
% 
% [pathstr, filename, fileext] = fileparts(soundvec);
...
num_samples = length(soundvec);
half_sample_rate = fs/2;


if opts.verbose > 0
  fprintf('%s:\n\t%i samples\n\tFs = %i\n\t%i bits\n', soundvec, num_samples, fs, bits);
end


% calculate input level, in terms of root sum squared
input_level = jp_rms(soundvec);
...
for i=1:num_channels
    ...
    analysisSounds(i,:) = filter(infilterB(i,:),infilterA(i,:),soundvec)';
