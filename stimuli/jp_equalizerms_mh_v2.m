function sound_norm = jp_equalizerms_mh_v2(sound_cell,verbose)
...
% 11/14/19 -- Now feeding in a 2D cell. --MH
...
for ii = 1:size(sound_cell, 1)
    for jj = 1:size(sound_cell, 2)
%     fileName = D(i).name;
...
%         [y,fs] = audioread(fullfile(sound_cell,fileName));
        y = sound_cell{ii, jj}(1, :); 
        rmsTotal = rmsTotal + rms(y);
        rmsCount = rmsCount+1;
    end
%     end
end % going through the first time

if rmsCount ~= numel(sound_cell); error('Check size'); end
